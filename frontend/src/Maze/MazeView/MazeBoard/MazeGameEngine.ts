...
export class MazeGameEngine {
  ...
  private setStatus : React.Dispatch<React.SetStateAction<'home' | 'animating' | 'animationPaused' | 'playing' | 'gamePaused' | 'finished'>>;// for handling the status of the maze its a State Variable 
  ...
  constructor(cellsData:mazeGridFromServer[][], context : CanvasRenderingContext2D, setStatus : React.Dispatch<React.SetStateAction<'home' | 'animating' | 'animationPaused' | 'playing' | 'gamePaused' | 'finished'>>, animationPath : path[] = []) {
  ...
  public animatingNextStep(){
    ...
    const prev = this.animationPath[this.currentAnimationCellIndex > 1 ? this.currentAnimationCellIndex - 1 : 0];
    ...
    this.context.clearRect(curr.col * this.cellSize, curr.row * this.cellSize, this.cellSize, this.cellSize);
    
    // adding the colour at a previous location then rendering boaders of that cell
    this.context.fillStyle = this.cellBackground; 
    this.context.fillRect(prev.col * this.cellSize, prev.row * this.cellSize, this.cellSize, this.cellSize);
    ...
    this.context.fillRect(curr.col*this.cellSize, curr.row*this.cellSize, this.cellSize, this.cellSize);
